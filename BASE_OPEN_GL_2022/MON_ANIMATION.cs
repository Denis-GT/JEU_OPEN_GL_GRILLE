...
namespace BASE_OPEN_GL
{
   ...
   partial class Program
   {
      ...
      static Stopwatch Chrono_FPS;
      static Stopwatch Chrono_Deplacement_Balle;
      ...
      static void Initialisation_Animation()
      {
         FG.FullScreen();
         Monde = new C_MONDE();
         Chrono_FPS = new Stopwatch();
         Chrono_Deplacement_Balle = new Stopwatch();
         Monde.Initialisation();
         Chrono_FPS.Start();
         Chrono_Deplacement_Balle.Start();
      ...
      static void Afficher_Ma_Scene()
      {
         ...
         double Interval_Deplaement = Monde.Configuration_Jeu.Interval_Deplacement_Personnage - (Math.Log(Monde.Score()+1) * 30);
         if (Chrono_Deplacement_Balle.ElapsedMilliseconds >= Interval_Deplaement) {
            Monde.Deplace_Balle();
            Chrono_Deplacement_Balle.Restart();
         }
         ...
         if (Chrono_FPS.ElapsedMilliseconds >= 1000) {
            IPS = Compteur_Images;
            Compteur_Images = 0;
            Chrono_FPS.Restart();
         }
      ...
      static void Gestion_Clavier(byte P_Touche, int P_X, int P_Y)
      {
         ...
         if (P_Touche == 122 || P_Touche == 90) Monde.Direcion_Balle = (0, 1);

         if (P_Touche == 115 || P_Touche == 83) Monde.Direcion_Balle = (0, -1);

         if (P_Touche == 100 || P_Touche == 68) Monde.Direcion_Balle = (1, 0);

         if (P_Touche == 113 || P_Touche == 81) Monde.Direcion_Balle = (-1, 0);
